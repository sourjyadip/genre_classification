#Architecture taken from 'Improved Music Genre Classification with Convolutional Neural Networks' 
import json
import numpy as np 
...
def build_model(input_shape):
    ...
    model.add(keras.layers.Conv2D(128, (4,4), strides=(4,4), activation = 'relu', input_shape = input_shape, padding ='same'))
    model.add(keras.layers.MaxPool2D((2,2), strides = (1,1), padding = 'same'))
    
    model.add(keras.layers.Conv2D(128, (4,4), strides=(1,1), activation = 'relu', input_shape = input_shape, padding ='same'))
    model.add(keras.layers.MaxPool2D((2,2), strides = (1,1), padding = 'same'))
    
    model.add(keras.layers.Conv2D(256, (4,4), strides = (1,1), activation = 'relu', input_shape = input_shape, padding ='same'))
    model.add(keras.layers.MaxPool2D((26,26), strides = (1,1), padding = 'same'))
...
print("Model built")
#compile the network
optimizer = keras.optimizers.Adadelta(learning_rate=1.0, rho=0.95, epsilon=1e-07)
